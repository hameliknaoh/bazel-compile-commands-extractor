...
def _parse_headers_from_makefile_deps(
    d_file_content: str, source_path_for_sanity_check: typing.Optional[str] = None
):
    ...
    assert "\r" not in d_file_content, (
        "Something went wrong in makefile parsing to get headers. Dependency file content should not contain literal '\r' characters. Output:\n"
        + repr(d_file_content)
    )
    ...
    target, dependencies = d_file_content.split(
        ": ", 1
    )  # Needs to handle absolute Windows paths, like C:\
    ...
    source, *headers = (
        dependencies  # The first dependency is a source entry, only used to (optionally) sanity-check the dependencies if a source path is provided.
    )
...
def _get_headers_gcc(compile_action, source_path: str, action_key: str):
    ...
    if (
        action_key in _get_bazel_cached_action_keys()
        ...
        for i, arg in enumerate(compile_action.arguments):
            if arg.startswith("-MF"):
                ...
                if os.path.isfile(dep_file_path):
                    ...
                    if _get_cached_adjusted_modified_time(
                        source_path
    ...
    header_cmd = (arg for arg in header_cmd if not arg.startswith("-fsanitize"))
...
def _subprocess_run_spilling_over_to_param_file_if_needed(
    ...
    try:
        ...
        if (
            e.winerror == 206
        ):  # Thrown when command is too long, despite the error message being "The filename or extension is too long". For a few more details see also https://stackoverflow.com/questions/2381241/what-is-the-subprocess-popen-max-length-of-the-args-parameter
            ...
            try:
                os.write(
                    fd, windows_list2cmdline(command[1:]).encode()
...
def _get_headers_msvc(compile_action, source_path: str):
    ...
    for line in header_search_process.stderr.splitlines():
        ...
        if (
            source_path.endswith("/" + line) or source_path == line
    ...
    if (
        error_lines
    ):  # Output all errors at the end so they aren't interlaced due to concurrency
        _print_header_finding_warning_once()
...
def _get_headers(compile_action, source_path: str):
    ...
    if {exclude_headers} == "all":
        ...
        and not {exclude_external_sources}
    ...
    for i, arg in enumerate(compile_action.arguments):
        ...
        if (
            ...
            arg.startswith("/Fo") or arg.startswith("-Fo")
    ...
    if output_file:
        cache_file_path = (
            output_file + ".hedron.compile-commands.headers"
    ...
    Continuing gracefully...""")
                ...
                if (
                    action_key == compile_action.actionKey
    ...
    if output_file and should_cache:
        ...
        not headers and cached_headers
...
def _get_files(compile_action):
    ...
    if len(source_file_candidates) > 1:
        ...
        if "-o" in compile_action.arguments:  # GCC, pre -o case
            ...
            assert "/c" in compile_action.arguments, (
                f"-o or /c, required for parsing sources in GCC or MSVC-formatted commands, respectively, not found in compile args: {compile_action.arguments}.\nPlease file an issue with this information!"
            )
    ...
    if not os.path.isfile(source_file):
        if (
            not _get_files.has_logged_missing_file_error
        ):  # Just log once; subsequent messages wouldn't add anything.
...
def _get_apple_DEVELOPER_DIR():
    ...
    return subprocess.check_output(
        ("xcode-select", "--print-path"), encoding=locale.getpreferredencoding()
    ).rstrip()
...
def _apple_platform_patch(compile_args: typing.List[str]):
    ...
    if any("__BAZEL_XCODE_" in arg for arg in compile_args):
        ...
        compile_args = [
            arg
            for arg in compile_args
            if not arg.startswith("DEBUG_PREFIX_MAP_PWD") or arg == "OSO_PREFIX_MAP_PWD"
...
def _all_platform_patch(compile_args: typing.List[str]):
    ...
    compile_args = (
        arg for arg in compile_args if not arg == "-fno-canonical-system-headers"
    )
...
def _get_cpp_command_for_files(compile_action):
    ...
    if (
        "PATH" not in compile_action.environmentVariables
...
def _get_commands(target: str, flags: str):
    ...
    if "--" in additional_flags[:-1] or any(
        re.match(r"-?(@|:|//)", f) for f in additional_flags
    ):
    ...
    if not getattr(
        parsed_aquery_output, "actions", None
